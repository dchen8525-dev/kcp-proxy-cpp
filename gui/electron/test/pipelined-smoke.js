...
//
// This is a MANUAL probe against a live tunnel (default 127.0.0.1:11080, or
// PORT env var). It is picked up by `npm test` (node --test), so it skips
// itself when nothing is listening instead of reporting a bogus failure.

const net = require('net');

const PORT = Number(process.env.PORT || 11080);

// Skip when no live tunnel is listening: `node --test` runs this file in
// every `npm test`, and a bare ECONNREFUSED there reads like a regression.
...
probe.once('error', () => {
  console.log(`SKIP: no live tunnel on 127.0.0.1:${PORT} (start the GUI proxy first)`);
  process.exit(0);
});

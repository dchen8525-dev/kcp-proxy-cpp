...
- Kernel UDP buffer ceiling: the server asks for 4 MiB socket buffers, but the kernel silently clamps
  them to `net.core.rmem_max`/`wmem_max` (208 KiB on Debian) while `setsockopt` still succeeds. Both
  installers write `/etc/sysctl.d/99-kcp-proxy.conf` (value from `common.sh`, which
  `tests/smoke/smoke_test.py` pins against `config.hpp`) **before** the restart, and **only when the
  host's current value is lower** — an operator's higher setting is never lowered. Not a conffile: the
  file is generated by postinst/`install-service.sh` and removed by postrm/`uninstall-service.sh`.
  Uninstall does not lower the live value (it stays until reboot); the uninstaller says so.
  The server and client compare requested vs. effective size at startup and log a WARNING when clamped
  (`describe_buffer_clamp` in `socket_buffer.hpp`).
...
- The server keeps `UDP_RECV_SLOTS` (16) `async_receive_from` operations outstanding on its one shared
  UDP socket, each with its own buffer + endpoint (`KCPServer::RecvSlot`). One outstanding receive
  couples the kernel drain rate to `route_datagram()`'s cost, so a simultaneous burst from many sessions
  overflows the socket buffer. Every completion handler is explicitly `bind_executor`-ed to the socket's
  strand (the socket is built on one), which is what keeps the plain, non-atomic receive-path state
  (`auth_attempts_window_`, the `LogThrottle`s, `recv_backoff_armed_`) safe with `-T > 1`. Unknown
  receive errors park their slot on a single **shared** backoff timer — per-slot timers would multiply
  the ~100 retries/s bound by 16.
